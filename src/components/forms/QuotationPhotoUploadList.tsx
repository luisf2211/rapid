...
import { QUOTATION_PHOTO_CATEGORIES } from "@/lib/constants";
import type { QuotationFormValues } from "@/lib/validations/quotation";

interface QuotationPhotoUploadListProps {
  control: Control<QuotationFormValues>;
  fields: FieldArrayWithId<QuotationFormValues, "photos", "id">[];
  append: UseFieldArrayAppend<QuotationFormValues, "photos">;
  remove: UseFieldArrayRemove;
  errors?: FieldErrors<QuotationFormValues>;
}

export function QuotationPhotoUploadList({
  control,
  fields,
  append,
  remove,
  errors,
}: QuotationPhotoUploadListProps) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {uploading ? "Subiendo..." : "Cargar fotos del vehículo"}
        </button>
        <p className="text-xs text-rapid-text-muted">
          JPG, PNG, WEBP o GIF · máx. 10 MB · aparecen en la impresión PDF
        </p>
      </div>
      ...
      {fields.length === 0 ? (
        <div className="border border-dashed border-rapid-border rounded-xl py-12 flex flex-col items-center justify-center text-rapid-text-muted gap-2">
          ...
          <p className="text-xs">Sube imágenes del daño o del vehículo para el presupuesto</p>
        </div>
      ) : (
      ...
      <div className="flex-1 p-3 space-y-2.5 min-w-0">
        ...
        <Controller
          ...
          render={({ field: f }) => (
            <SelectInput
              label="Categoría"
              options={QUOTATION_PHOTO_CATEGORIES}
              {...f}
            />
          )}
        />
